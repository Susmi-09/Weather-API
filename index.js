document.getElementById('getWeatherBtn').addEventListener('click', function () {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&units=metric`)
        ...
        .then(data => {
            ...
            const iconUrl = `http://openweathermap.org/img/wn/${iconCode}@2x.png`; // Corrected URL string
        ...
        .catch(error => {
            document.getElementById('weatherResult').innerHTML = `<p>${error.message}</p>`; // Corrected template string
        });
});
...
function changeBackground(weatherMain) {
    ...
    document.body.style.backgroundImage = `url(${backgroundUrl})`; // Corrected background image setting
